...
# 定义每个方向的阈值
THRESHOLD_FRONT = 0.5
THRESHOLD_FRONT_LEFT = 0.55
THRESHOLD_FRONT_RIGHT = 0.55
...
class MecanumWheelController(Node):
    ...
    def laser_callback(self, msg):
        ...
        if front < THRESHOLD_FRONT:
            if front_left < THRESHOLD_FRONT_LEFT:
...
def main(args=None):
    ...
    driver_node = MecanumWheelController()

    try:
        rclpy.spin(node=driver_node)
    except KeyboardInterrupt:
        pass
    finally:
        print(f'\nStopping...')
        driver_node.stop_all_motors()
